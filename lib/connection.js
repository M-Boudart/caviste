    ...
    if (sessionStorage.length == 0) { // Utilisateur déconnecté
        ...
        document.querySelector("#btAddComment").style.display = 'none';
        ...
        document.querySelector("#btAddComment").style.display = 'inline';

        const btLogOut = document.querySelector('#btLogOut');
        // Gestion de la déconnexion
        btLogOut.addEventListener('click', function() {
            sessionStorage.clear();
            window.location.href = 'http://localhost/caviste/'; // TODO Enlever évènement "click" dans likeManagement.js (lg23) sachant que fct anonyme (Maxime) car solution rustine
        });
    }
